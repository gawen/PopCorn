...
/* 
This sample demonstrates providing a procedural texture to a terrain. 
...
// Create the terrain
	try
	{
		...
		// Load a terrain that was created in the Demeter Texture Editor.
		pTerrain = new Terrain(MAX_NUM_VISIBLE_TRIANGLES,0.0f,0.0f);
		//TerrainLoader loader;
		//loader.LoadTerrain(pTerrain,"Llano.terrain",MAX_NUM_VISIBLE_TRIANGLES);
		// If we wanted to load directly from image or SDTS, DEM, DTED, ArcGRID, etc. files instead of from the Demeter Editor, we would do something like this:
		//pTerrain = new Terrain("LlanoElev.jpg","LlanoTex.jpg","dirt2.jpg",20.0f,3.0f,MAX_NUM_VISIBLE_TRIANGLES);
#ifdef _DEBUG
...
#endif
		pTerrain->AllocateTextureCells(8, 8);
		pTerrain->SetTextureFactory(new SampleTextureFactory(pTerrain));
	}
