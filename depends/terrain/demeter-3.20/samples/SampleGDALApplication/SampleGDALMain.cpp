...
/* 
This sample shows how to use GDAL to load elevation data. In this example,
we load elevation data from a grayscale image file (a .PNG file in this case.) By
simply changing the filename and MediaPath, you can experiment with loading
DEM, SDTS, ArcGRID, or any of the other formats supported by GDAL.
		...
		while (continueRunning)
		{
			...
			if (500 < elapsedTime)
			{
				...
				if (bAutomaticallyAdjustForTargetFrameRate && 2.0f < (float)fabs(TARGET_FRAME_RATE - currentRenderSpeed))
				{
					...
					cout << "VIEWER: Auto threshold = " << threshold << endl;
				}
			}
			cycles++;
		}
